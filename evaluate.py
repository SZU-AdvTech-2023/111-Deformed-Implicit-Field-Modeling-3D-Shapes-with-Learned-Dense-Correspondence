...
#     param.requires_grad = False
for param in model.hyper_net_actuated.parameters():
    param.requires_grad = False
for param in model.hyper_net_deform.parameters():
    param.requires_grad = False
...
for file in all_names:
    ...
    # shape embedding
    training_loop.train(model=model, train_dataloader=dataloader, model_dir=save_path, is_train=False, name=file, **meta_params)
