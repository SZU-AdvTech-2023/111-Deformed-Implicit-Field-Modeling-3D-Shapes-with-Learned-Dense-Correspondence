...
class PointCloud_with_FreePoints(Dataset):
    def __init__(self, pointcloud_path, on_surface_points, instance_idx=None, expand=-1, max_points=-1):
        ...
        # surface points
        surface_point_cloud = loadmat(pointcloud_path)
        point_cloud = surface_point_cloud['p']
        ...
        free_points_sdf = free_points[:,3:4]
        ...
        category_dict = {'S': 0, 'A': 1} # three gripper: shadowhand, rutgerhand, allegrohand
        self.category_idx = category_dict[re.split('/', pointcloud_path)[-1][0]]

        self.configuration = surface_point_cloud['joint_configs'].reshape(-1,1)
    ...
    def __getitem__(self, idx):
        ...
        'category_idx':torch.Tensor([self.category_idx]).squeeze().long()
        }
