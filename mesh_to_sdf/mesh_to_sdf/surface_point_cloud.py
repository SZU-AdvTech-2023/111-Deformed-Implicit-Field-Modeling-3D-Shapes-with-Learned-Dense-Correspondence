...
class SurfacePointCloud:
    def __init__(self, mesh, points, normals=None, scans=None, face_indices=None):
        self.mesh = mesh
        self.points = points
    ...
    def get_sdf(self, query_points, use_depth_buffer=False, sample_count=11, return_gradients=False):
        ...
        if return_gradients:
            ...
            return distances, indices, gradients
        else:
    ...
    def get_sdf_in_batches(self, query_points, use_depth_buffer=False, sample_count=11, batch_size=1000000, return_gradients=False):
        ...
        if return_gradients:
            ...
            indices = np.concatenate([batch[1] for batch in batches])
            gradients = np.concatenate([batch[2] for batch in batches])
            return distances, indices, gradients
        else:
            distances = np.concatenate([batch[0] for batch in batches])
            indices = np.concatenate([batch[1] for batch in batches])
    ...
    def sample_sdf_near_surface(self, number_of_points=500000, use_scans=True, sign_method='normal', normal_sample_count=11, min_size=0, return_gradients=False):
        ...
        print(surface_sample_count, unit_sphere_sample_count)
        ...
        if return_gradients:
            sdf, indices, gradients = sdf
        else:
            sdf, indices = sdf
        if self.face_indices is not None:
            indices = self.face_indices[indices]
        else:
            assert False
        ...
        if return_gradients:
            return query_points, sdf, indices, gradients
        else:
            return query_points, sdf, indices
